...
class OzonApi
{
    ...
    /*public function postsParallel(Carbon $dateSince) { 
        $dates = $this->dates($dateSince);
        $postings = [];
        foreach ($dates as [$dateSince, $dateTo]) {
            $p = new \parallel\Runtime();
            $future = $p->run(function() use ($dateSince, $dateTo) {
                dump($dateSince->toRfc3339String(), $dateTo->toRfc3339String());
                $postings = $this->postsDate($dateSince, $dateTo);
                return $postings;
            });

            $postings = array_merge($postings, $future->value());
        }
